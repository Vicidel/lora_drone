        ...
        if(current_state.mode != "OFFBOARD" && (ros::Time::now() - last_request > ros::Duration(5.0))){
            ...
            if(!current_state.armed && (ros::Time::now() - last_request > ros::Duration(5.0))){
                if(arming_client.call(arm_cmd) && arm_cmd.response.success){
                    ...
                    send_GPS(pos_drone, ros::Time::now().toSec(), (char*)"drone_armed");
                }
                last_request = ros::Time::now();
            }
                ...
                switch(state){
                    ...
                    case 1:{
                        if((pos_drone-pos_current_goal).norm()<precision){
                            ...
                            send_GPS(pos_drone, ros::Time::now().toSec(), (char*)"waypoint1_reached");
                            state = 2;
                        }
                        break;
                    }
                    ...
                    case 3:{
                        if((pos_drone-pos_current_goal).norm()<precision){
                            ROS_INFO("Goal reached!");
                            send_GPS(pos_drone, ros::Time::now().toSec(), (char*)"waypoint2_reached");
                            state = 4;
                        }
                        break;
                    }
                    ...
                    case 5:{
                        if((pos_drone-pos_current_goal).norm()<precision){
                            ROS_INFO("Goal reached!");
                            send_GPS(pos_drone, ros::Time::now().toSec(), (char*)"waypoint3_reached");
                            state = 6;
                        }
                        break;
                    }
